...
data PartialFunctionError = EmptyList String deriving (Eq)
...
-- | Input errors
data InputError = NonNegError String Int deriving (Eq)
...
data OutOfBoundsIndexError i = OOBIxError String i
                             ...
                             | OOBNoCompatRows String (i,i) deriving (Eq)
...
data OperandSizeMismatch = DotSizeMismatch Int Int
                         ...
                         | MatVecSizeMismatchException String (Int, Int) Int deriving (Eq)
...
data MatrixException i = HugeConditionNumber String i
                       | NeedsPivoting String String deriving (Eq)
...
instance Show a => Show (IterationException a) where
  ...
  show (IterE s1 s2) = unwords [s1, ":", s2]
